...
class GaussianDiffusion_Nolatent(nn.Module):
    ...
    def p_sample_loop_repaint(self,
        ...
        for sample_dict in self.p_sample_loop_repaint_progressive(
                shape,
                noise=noise,
            ...
            if return_intermediate:
                final = sample_dict['sample']
                if 'initial_noise' in sample_dict and sample_dict['initial_noise'] is not None:
                    initial_noise = sample_dict['initial_noise']
                if 'noised_input' in sample_dict and sample_dict['noised_input'] is not None:
                    noised_input = sample_dict['noised_input']
                if 'denoised_output' in sample_dict and sample_dict['denoised_output'] is not None:
                    denoised_output = sample_dict['denoised_output']
            else:
                final = sample_dict
        ...
        if self.data_type == 'emidec':
            ...
            final1, final2 = torch.split(final, final.size(1) // 2, dim=1)
            final = final1 * mask1 + final2 * mask2 + final1 * mask
        elif self.data_type == 'pfoa_pdw_atse':
            gt_keep_mask = model_kwargs.get('gt_keep_mask')
            mask = ((gt_keep_mask == 3) | (gt_keep_mask == 4)).float()  
            mask = mask.eq(0) 
            mask1 = (gt_keep_mask == 3).float() 
            ...
            final1, final2 = torch.split(final, final.size(1) // 2, dim=1)
            final = final1 * mask1 + final2 * mask2 + final1 * mask
        elif self.data_type in ['pfoa_normal', 'pfoa_normal_pdw_atse', 'pfoa_normal_t1w']:
            gt_keep_mask = model_kwargs.get('gt_keep_mask')
            mask = ((gt_keep_mask == 3) | (gt_keep_mask == 4)).float()  
            mask = mask.eq(0) 
            mask1 = (gt_keep_mask == 3).float() 
    ...
    def p_sample_repaint(
        ...
        noised_input_xt = x.clone() if return_intermediate else None
        
        if isinstance(self.denoise_fn, torch.nn.DataParallel):
            noise_pred = self.denoise_fn.module.forward_with_cond_scale(x, t, cond=cond, cond_scale=cond_scale)
        else:
            noise_pred = self.denoise_fn.forward_with_cond_scale(x, t, cond=cond, cond_scale=cond_scale)
        ...
        denoised_output_x0 = x_recon.clone() if return_intermediate else None
        ...
        if return_intermediate:
            return output, noised_input_xt, denoised_output_x0
    ...
    def p_sample_loop_repaint_progressive(
        ...
        initial_noise = image_after_step.clone() if return_intermediate else None
        
        saved_noised_input = None
        saved_denoised_output = None
        ...
        if conf.schedule_jump_params:
            ...
            if save_timestep is None:
                save_timestep = len(time_pairs) // 2 
            
            if progress:
                from tqdm.auto import tqdm
                time_pairs = tqdm(time_pairs)
            
            for step_idx, (t_last, t_cur) in enumerate(time_pairs):
                idx_wall += 1
                t_last_t = t_last
                if t_cur < t_last: 
                    with (torch.no_grad()):
                        ...
                        should_save = return_intermediate and (step_idx == save_timestep)
                        
                        out, noised_input_xt, denoised_output_x0 = self.p_sample_repaint(
                            image_after_step, 
                            ...
                            conf=conf,
                            cond=cond,
                            return_intermediate=should_save
                        )
                        
                        if should_save:
                            saved_noised_input = noised_input_xt
                            saved_denoised_output = denoised_output_x0
                        ...
                        if return_intermediate:
                            yield {
                                'sample': out,
                                'initial_noise': initial_noise,
                                'noised_input': saved_noised_input,
                                'denoised_output': saved_denoised_output
                            }
                        else:
                            yield out
                else:
