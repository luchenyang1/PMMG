...
class ModelWrapper(torch.nn.Module):
    def __init__(self, model, input, clinic, report, prompt, branch = 0, img_indx = 0, slice_indx = 0, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        if torch.cuda.is_available:
            input = [image.cuda() for image in input]
            clinic = clinic.cuda()
            report = report.cuda()
            prompt = prompt.cuda()
        ...
        self.slice_indx = slice_indx
        self.branch = branch 
        self.target_layer = model.sag_PDW_spair_enc.model.layer4[-1]
    ...
    def forward(self, input_img):
        ...
        clinic = self.clinic.unsqueeze(0)
        report = self.report.unsqueeze(0)
        prompt = self.prompt.unsqueeze(0)
        output = self.model(images, clinic, report, prompt)
...
def show_CAM(net, dataset):
    # cam args
    target_task = 1
    images, label, clinic, report, prompt, name = dataset[6]
    output_branch = 0 # final_pred, sag_pred, cor_pred, axi_pred 
    input_image_indx = 0 # sag_img, cor_img, axi_img, t2_img, t1_img
    # slice_indx = 112
    for task in range(1):
        target_task = task
        for i in range(0, 220, 10):
            slice_indx = i
            model = ModelWrapper(model=net, input=images, clinic=clinic, report=report, prompt=prompt, branch=output_branch, img_indx = input_image_indx)
            ...
            plt.imsave("./show_CAM/task_%d_slice_%d.png"%(task, slice_indx), visualization)
            Show_Samples(input_image, title="task_%d_slice_%d_origin.png"%(task, slice_indx), save_path='./show_CAM')
        # cam_map.shape()
...
def val_with_save(net, test_dataset):
    ...
    with torch.no_grad():
        test_loss, test_auc, test_acc = evaluate_model(net, test_loader, 1, args, mode="Test", save_path=log_root_folder, save_results=True)
