...
def doctor_eval(file_path, write_output=True, save_path = None):
    import pandas as pd
    if write_output:
...
def evaluate_prediction(prediction:list, label:list, metrix_output=False, args=Arguments()):
    ...
    for i, task in enumerate(args.DiseaseList):
        preds = torch.softmax(torch.tensor(prediction[:, i * 4:(i + 1) * 4]), dim=1)
        trues = label[:, i]
        auc_dict[task], acc_dict[task], metrix_dict[task] = summary(trues, preds, task_type='multiclass')
...
def summary(label, prob_pred, task_type='multiclass'):
    if task_type == 'binary':
        pred = np.ones_like(prob_pred) * (prob_pred >= 0.5)
    elif task_type == 'multiclass':
        pred = torch.argmax(prob_pred, dim=1).unsqueeze(1) 
        pred = np.array(pred)
    ...
    try:
        ...
        confusion = np.array2string(confusion, separator=',')
        # TN, FP, FN, TP = confusion.ravel()
        TN, FP, FN, TP = [-1, -1, -1, -1]
    except:
        TN, FP, FN, TP = [-1, -1, -1, -1]

    precision = precision_score(label, pred, average="macro", labels=[0, 1, 2, 3])
    recall = recall_score(label, pred, average="macro", labels=[0, 1, 2, 3])
    f1 = f1_score(label, pred, average="macro", labels=[0, 1, 2, 3])
    
    try:
        auc = roc_auc_score(label, prob_pred, multi_class='ovr')
    except:
        auc = -1
    
    # return auc, acc, [balance_acc, TN, FN, TP, FP, precision, recall, f1, auc, acc]
    return auc, acc, [balance_acc, confusion, precision, recall, f1, auc, acc]


def write_metrix(metrix_dict, save_path, view, args=Arguments(), overwrite=False, savename="metrix"):
    per_class_metric_name = ['Task', "balance_acc", "confusion", "precision", "recall", "f1", "auc", "acc"]
